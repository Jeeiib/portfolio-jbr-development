...
const SLUG = "developpeur-web-lille";
...
export default async function DeveloppeurWebLillePage({ params }: PageProps) {
  const { locale } = await params;
  ...
  // Schema.org FAQPage - Safe: all content is from static translation files
  const faqSchema = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: faqItems.map((item) => ({
      "@type": "Question",
      name: t(`${SLUG}.faq.${item.key}.question`),
      acceptedAnswer: {
        "@type": "Answer",
        text: t(`${SLUG}.faq.${item.key}.answer`),
      },
    })),
  };
      ...
      <main className="pt-20">
        ...
        <section className="py-16 md:py-24 bg-[var(--background)] relative overflow-hidden">
          ...
          <div className="section-container relative z-10">
            <div className="max-w-3xl">
              ...
              <p className="text-xl text-[var(--foreground-secondary)] mb-6 leading-relaxed">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <Link
                  href={`/${locale}#projets`}
                  className="inline-flex items-center justify-center px-8 py-4 border border-[var(--border)] font-semibold rounded-lg hover:border-[var(--accent)] hover:text-[var(--accent)] transition-colors"
                >
                  {tCommon("cta.projects")}
          ...
          <div className="section-container">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {whyChooseMe.map((item) => (
                <div
                  key={item.key}
                  className="group p-6 bg-[var(--background)] border border-[var(--border)] rounded-2xl transition-all duration-300 hover:border-[var(--accent)] hover:-translate-y-1"
                >
                  <div className="w-12 h-12 flex items-center justify-center rounded-xl bg-[var(--accent)]/10 text-[var(--accent)] mb-4 group-hover:bg-[var(--accent)] group-hover:text-[var(--background)] transition-all duration-300">
                    {whyIcons[item.icon]}
                  </div>
                  <h3 className="text-lg font-semibold mb-2 group-hover:text-[var(--accent)] transition-colors">
                    {tCommon(`whyMe.items.${item.key}.title`)}
                  </h3>
                  <p className="text-[var(--foreground-secondary)] text-sm leading-relaxed">
                    {tCommon(`whyMe.items.${item.key}.description`)}
                  </p>
                </div>
              ))}
            </div>
        ...
        <section className="py-16 md:py-24 bg-[var(--background)]">
          <div className="section-container">
            <div className="text-center mb-12">
              <span className="inline-block text-sm font-medium text-[var(--accent)] uppercase tracking-widest mb-4">
                {tCommon("services.label")}
              </span>
              <h2 className="text-3xl sm:text-4xl font-bold">
                {tCommon("services.title")}
              </h2>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {["siteVitrine", "applicationWeb", "landingPage"].map((service) => (
                <div
                  key={service}
                  className="group flex flex-col p-8 bg-[var(--background-card)] border border-[var(--border)] rounded-2xl transition-all duration-300 hover:border-[var(--accent)]"
                >
                  <div className="w-14 h-14 flex items-center justify-center rounded-xl bg-[var(--accent)]/10 text-[var(--accent)] mb-6 group-hover:bg-[var(--accent)] group-hover:text-[var(--background)] transition-all duration-300">
                    {serviceIcons[service === "siteVitrine" ? "globe" : service === "applicationWeb" ? "layout" : "rocket"]}
                  </div>
                  <h3 className="text-xl font-semibold mb-3 group-hover:text-[var(--accent)] transition-colors">
                    {tCommon(`services.items.${service}.title`)}
                  </h3>
                  <p className="text-[var(--foreground-secondary)] mb-4 leading-relaxed">
                    {tCommon(`services.items.${service}.description`)}
                  </p>
                  <ul className="space-y-2 mb-6 flex-grow">
                    {[1, 2, 3, 4].map((i) => (
                      <li key={i} className="flex items-center text-sm text-[var(--foreground-secondary)]">
                        <svg className="w-4 h-4 text-[var(--accent)] mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                        </svg>
                        {tCommon(`services.items.${service}.features.${i}`)}
                      </li>
                    ))}
                  </ul>
        ...
        <section id="contact" className="py-16 md:py-24 bg-[var(--background)]">
          <div className="section-container">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
              ...
              <div>
                <span className="inline-block text-sm font-medium text-[var(--accent)] uppercase tracking-widest mb-4">
                  {tCommon("contact.label")}
                </span>
                <h2 className="text-3xl sm:text-4xl font-bold mb-6">
                  {tCommon("contact.title")}
                </h2>
                <p className="text-[var(--foreground-secondary)] mb-8 leading-relaxed">
                  {tCommon("contact.subtitle")}
                </p>

                {/* Contact Info */}
                <div className="space-y-4 mb-8">
                  <div className="flex items-center gap-4 p-4 bg-[var(--background-secondary)] border border-[var(--border)] rounded-xl">
                    ...
                    <div>
                      <p className="text-sm text-[var(--foreground-secondary)]">{tCommon("contact.address")}</p>
                      <p className="font-medium">Lambersart (Lille métropole)</p>
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <div>
                      <p className="text-sm text-[var(--foreground-secondary)]">{tCommon("contact.phone")}</p>
                      <p className="font-medium">06 18 97 22 50</p>
                    </div>
                  </a>
                </div>

                {/* Google Rating */}
                <div className="inline-flex items-center gap-2 px-4 py-2 bg-[var(--background-secondary)] border border-[var(--border)] rounded-full">
                  <div className="flex text-yellow-400">
                    {[...Array(5)].map((_, i) => (
                      <svg key={i} className="w-4 h-4 fill-current" viewBox="0 0 20 20">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                  ...
                  <span className="text-sm font-medium">5/5 {tCommon("contact.onGoogle")}</span>
                </div>
              </div>

              {/* Right - Form */}
              <div>
                <ContactForm source={SLUG} />
              </div>
            </div>
          </div>
        </section>
